...
export function TheTeacherRecentActivity() {
    ...
    const fetchActivity = async () => {
        try {
            ...
            // Fetch recent papers from user repo
            if (user) {
    ...
    return (
        <div className="space-y-5">
            ...
            {loading ? (
                ...
                <div className="space-y-3">
                                        ...
                                        <div className="flex items-center gap-4 relative z-10 min-w-0 flex-1">
                                            ...
                                            <div className="min-w-0 flex-1">
                                                ...
                                                <div className="flex items-center gap-2 mt-0.5">
                                                    ...
                                                    <span className={`inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full border capitalize ${status.bg} ${status.color}`}>
                                                        {status.icon} {exam.status}
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
                    ...
                    {papers.map((paper, i) => (
                        <motion.div
                            ...
                            initial={{ opacity: 0, x: -12 }}
                            animate={{ opacity: 1, x: 0 }}
                            transition={{ duration: 0.3, delay: (exams.length + i) * 0.06 }}
                        >
                            <div
                                onClick={() => router.push(`/teacher-dashboard/paper/${paper.id}`)}
                                className="group bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between hover:border-indigo-200 hover:shadow-md transition-all duration-300 cursor-pointer relative overflow-hidden"
                            >
                ...
                <div className="relative bg-gradient-to-br from-slate-50 to-white p-10 rounded-2xl border-2 border-dashed border-slate-200 text-center overflow-hidden group hover:border-purple-200 transition-colors">
                    ...
                    <div className="relative z-10">
                        ...
                        <div className="flex gap-3 justify-center">
                            <Link
                                href="/teacher-dashboard/custom-generate"
                            ...
                            <Link
                                ...
                                className="inline-flex items-center gap-2 px-4 py-2.5 bg-white text-slate-700 text-sm font-semibold rounded-xl border border-slate-200 hover:border-indigo-200 hover:shadow-md transition-all"
                            >
