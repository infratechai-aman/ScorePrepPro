...
// ====================================================================
// MAIN PIPELINE — Parallel high-quality generation
// ====================================================================
...
export async function generatePremiumNotes(
    ...
    const report = (step: PipelineStep, num: number, msg: string) => {
        onProgress?.({ step, stepNumber: num, totalSteps: 3, message: msg });
    };

    report("analyzing", 1, "Analyzing content and preparing parallel generation...");
    
    const baseSource = knowledgeText.substring(0, 25000);

    report("generating", 2, "Generating massive high-quality notes & diagrams (this takes ~30 seconds)...");

    // We do 3 parallel calls to get maximum length and detail without hitting the 60s timeout limit.
    const [part1, part2, part3] = await Promise.all([
        // CALL 1: Core Concepts & Theory
        openai.chat.completions.create({
            model: "gpt-4o",
            messages: [
                { role: "system", content: ELITE_WRITER_SYSTEM },
                { role: "user", content: `Generate Part 1 (Core Theory) of premium notes for:
Subject: ${subjectName}
Unit: ${unitTitle}
...
REQUIREMENTS:
1. Start with a compelling chapter snapshot and 4-5 learning objectives.
2. Dive deep into the Core Concepts. Explain them thoroughly.
3. Provide deep conceptual clarity, not just surface-level bullet points.
4. Use "> **Key Insight**: " for breakthrough moments.
5. Use proper markdown headings (##, ###).
6. DO NOT write an introduction or conclusion. Start immediately with the title and content.`}
            ],
            temperature: 0.6,
            max_tokens: 4000,
        }),

        // CALL 2: Definitions, Formulas, Tables, & Exam Focus
        openai.chat.completions.create({
            model: "gpt-4o",
            messages: [
                { role: "system", content: ELITE_WRITER_SYSTEM },
                { role: "user", content: `Generate Part 2 (Details & Exam Focus) of premium notes for:
Subject: ${subjectName}
Unit: ${unitTitle}
...
REQUIREMENTS:
1. Start with ## Important Definitions. Use "> **Definition** — *Term*: meaning".
2. Next, ## Formulas & Equations. Use \`code blocks\` and list variable meanings.
3. Next, ## Comparison Tables. If concepts can be contrasted, you MUST use Markdown tables.
4. Next, ## Exam Focus Areas. Add 3-5 likely exam questions with model answer hints.
5. End with ## Revision Summary.
6. DO NOT write an introduction. Start immediately with the first heading.`}
            ],
            temperature: 0.5,
            max_tokens: 4000,
        }),

        // CALL 3: Diagrams & Visuals
        openai.chat.completions.create({
            model: "gpt-4o",
            messages: [
                { role: "system", content: `You create text-based academic diagrams using ASCII art and structured markdown. Use code blocks for diagrams.` },
                { role: "user", content: `Generate Part 3 (Diagrams) for:
Subject: ${subjectName}
Unit: ${unitTitle}
...
REQUIREMENTS:
1. Create 2-3 visual diagrams or concept maps for the most important topics in this unit.
2. For each diagram, create a clear ### heading, the diagram in an ASCII code block, and a 1-2 line description below.
3. Use boxes (┌──┐), arrows (──▶), and lines to show flow or relationships.
4. Start with: ## Diagrams & Concept Maps`}
            ],
            temperature: 0.4,
            max_tokens: 2000,
        })
    ...
    const content1 = part1.choices[0].message.content || "";
    const content2 = part2.choices[0].message.content || "";
    const content3 = part3.choices[0].message.content || "";

    // Assemble the final massive document
    let finalNotes = `# ${unitTitle}\n\n`;
    finalNotes += `**Subject**: ${subjectName} | **Premium Generated Notes**\n\n---\n\n`;
    finalNotes += `${content1}\n\n---\n\n`;
    finalNotes += `${content2}\n\n---\n\n`;
    finalNotes += `${content3}\n\n---\n\n`;
    finalNotes += `*Generated by ScorePrepPro — Premium AI Notes Engine*\n`;
